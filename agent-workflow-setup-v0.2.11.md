# The ORCA ADE agent workflow: complete setup (v0.2.11)
...
| CI job dies on the session limit | the CI agents run on the same subscription as the local ones, so they fail at the same moment the local runs do. A run that failed inside the hold window is re-run once the hold lifts, and that attempt is not counted (v0.2.7). |
