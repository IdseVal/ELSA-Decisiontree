# The ORCA ADE agent workflow: complete setup (v0.2.7)
...
| CI run for its current head FINISHED, and nothing was decided (v0.2.7) | three minutes after the run ended, re-runs the job that owes the verdict with `gh run rerun`: `review` when `state:tested` is on (the Verifier decided, the Reviewer did not), else `verify`. One breaker cycle per attempt; `escalated` past the breaker; the attempt is not counted when the run failed inside a session-limit hold, and nothing is re-run while the hold lasts. A refused re-run pages `needs-human`. |
...
GitHub is still queueing it. Each attempt spends a breaker cycle on the linked issue;
past the breaker the issue is `escalated` with a comment on the PR saying what to do
(re-run from the Actions tab, or remove `escalated` for a fresh count). `gh run rerun`
...
And since v0.2.7 the CI re-runs spend cycles too: each time the dispatcher re-runs a
finished workflow run that decided nothing, the linked issue's count goes up by one, so a
pull request whose Reviewer stays silent escalates after three re-runs instead of being
re-run forever at the price of a full review each time. A re-run after a session-limit
death is free, like the local kind.
...
itself, up to three times, then escalates it to you like any other stuck task. You do
