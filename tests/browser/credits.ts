...
/** One picture on a Node's page, in the Carousel's order, with the Option it belongs to if any. */
...
/**
 * Every Node of the Tree in `treeDir` that shows a picture, with its pictures in strip order:
 * its own Images, then the first Image of each Option, the one on the Branch (12.1). The ids
 * are read off the file only to enumerate; every Node comes through the loader (section 7).
 */
export async function picturesByNode(tree: Tree, treeDir: string, lang: string): Promise<Map<string, Picture[]>> {
...
export async function readEveryCredit(page: Page, url: string, pictures: Picture[]): Promise<number> {
  ...
  for (const [index, { image, option }] of pictures.entries()) {
